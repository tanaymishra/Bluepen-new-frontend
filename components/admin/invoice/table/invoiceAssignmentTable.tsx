...
const InvoiceAssignmentTable: React.FC<InvoiceAssignmentTableProps> = ({
        ...
        },
        {
            ...
            cellRenderer: (params: any) => (
                <div
                    ...
                    ${(params.data.status === 'Assigned to Freelancer') ? css.assignedToFreelancer : ''}
                    ${(params.data.status === 'Failed' || params.data.status === 'Resit') ? css.failed : ''}
                    ${params.data.status === 'Lost' ? css.lost : ''}
                    `}
    ...
    return (
        <div className={css.tableWrapper}>
                            ...
                            options={[
                                {
                                    component: <Legend
                                        items={[
                                            ...
                                            { color: 'rgb(150, 18, 18)', label: 'Failed / Resit' },
            ...
            <div className={`ag-theme-alpine ${css.gridContainer}`}>
                <AgGridReact
                    rowData={rowData}
                    ...
                    rowClassRules={{
                        [css.disabledRow]: (params) => {
                            return !((params.data.status === 'Completed Marks Not Received' || params.data.status === 'Completed' || params.data.status === 'Review Received' || params.data.status === 'Resit' || params.data.status === 'Failed') &&
